...
typedef enum { AV , TG , TD, NUL } Ordre // 0 correspond à aucun ordre
...
void determinerOrdres(char tabOrdres[MAX_ORDRES] , unsigned int plusCourtChemin[MAX_CASES] , unsigned int nbCaseChemin, 
                        unsigned int debut, unsigned int fin, 
                        Robot robot , 
    ...
    for (i = 0, i < nbCaseChemin - 1, i++) {
        determinerOrdre(plusCourtChemin[i], plusCourtChemin[i+1], getDirection(robot), largeurCircuit, ordre1, ordre2)
        if (ordre1 == NUL) 
            tabOrdres[j] = ordre2;
        else {
            tabOrdres[j] = ordre1;
...
void determinerOrdre(unsigned int debut, unsigned int fin 
                    tDirection dirRobot,
                    unsigned int largeurCircuit, 
                    Ordre ordre1, Ordre ordre2) {
