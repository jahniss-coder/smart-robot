/**
 * @file graphe_dijkstra.h
 * @brief Fichier d'en-tête pour la création du graphe de la ville et la résolution du plus court chemin avec l'algorithme de Dijkstra.
 * @author Delacroix-Henrion Mathieu, Diallo Alioune, Malherbe Ylann, Mathieu Ilan, Olivier Neil, Schetrit Jahnis
 * @date 2025-12
 */

#ifndef __GRAPHE_DIJKSTRA__
#define __GRAPHE_DIJKSTRA__
#include "graphe.h"
#include "TAD_robot.h"
#include "chemin.h"

#define MAX_CHAINE 3          /**< Nombre maximum de points dans une chaîne. */
#define NB_POINTS_MAX 200     /**< Nombre maximum de points dans le graphe. */
#define MAX_CASES_OBLIGATOIRES 3 /**< Nombre maximum de cases obligatoires à visiter. */

/**
 * @brief Structure représentant l'état actuel de l'algorithme de Dijkstra.
 */
typedef struct {
...
/**
 * @brief Structure représentant la solution du problème du plus court chemin.
 */
typedef struct {
    unsigned int ordre[3];        /**< Ordre dans lequel on va aller aux 3 points obligatoires. */
    Chemin chemin_complet;       /**< Tableau avec tous les points par lesquels on doit passer. */
} Solution;

/**
 * @brief Récupère toutes les informations contenues dans le fichier de la ville.
 *
 * @param fileNameVille Nom du fichier contenant les informations de la ville.
 * @param l Pointeur vers la largeur de la ville.
 ...
 * @param nbLiaisons Pointeur vers le nombre de liaisons dans la ville.
 * @param nbCasesObligatoires Pointeur vers le nombre de cases obligatoires.
 * @param caseInitRobot Pointeur vers la case initiale du robot.
 * @param orientationInitRobot Pointeur vers la direction initiale du robot.
 */
...
/**
 * @brief Crée un graphe à partir d'un tableau de liaisons.
 *
 ...
 * @param directionInitialeRobot Direction initiale du robot.
