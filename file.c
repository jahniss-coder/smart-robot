...
void enfiler(File* f, pointDijkstra pD){
    ...
    if !estPleineFile(f){
        f->elements[f->fin] = pD;  // Car au depart fin = 0 donc on veut bien ajouer en position 0 en premier
...
pointDijkstra defiler(File* f){
    ...
    if !estVideFile(f){
        pointDijkstra pD = f->elements[f->debut];   // On défile le premier élement
